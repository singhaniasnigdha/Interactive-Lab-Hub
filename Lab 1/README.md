...
The set up for the interaction video is identical to what was used earlier (in part 1). We hosted a Zoom meeting, and shared the screen allowing the laptop to control the images/videos showing up in the Hologram. We also transmitted voice through the call, that conveyed the errors or sentiments associated with those emoticons. The animated emoticons were built using Apple Memoji, Tenor (GIF search engine) and Microsoft Powerpoint.
...
While the interaction showcases responses to an email, HoloSent's use case can be expanded well outside emails, to private messaging and blogs too (as suggested by Sam Lee). 
